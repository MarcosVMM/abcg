...
void OpenGLWindow::initializeGL() {
  ...
  if(animal==1){
  // Load default model
  loadModel((getAssetsPath() + "catzinho.obj"),animal);
  }else if(animal == 2){
    loadModel((getAssetsPath() + "bunny.obj"),animal);
  }*/
  /*
...
void OpenGLWindow::paintUI() {
  ...
  if (start == true){
   auto size{ImVec2(300, 85)};
    ...
    ImGui::Text("Selecione um animal ao lado \n para visualizar!");
    ImGui::End();
  }
...
// Only in WebGL
 ...
 ImGuiWindowFlags flags{
    ...
    //Alternativas
static std::size_t currentIndex{};
  ImGui::Begin("Animais",nullptr, flags);
  ImGui::Text("Selecione!");
  ImGui::Columns(1, NULL, false);
  if(ImGui::Button("Gato", ImVec2(70, 50))){
        currentIndex = 0;
        m_currentProgramIndex = currentIndex;
        m_model.setupVAO(m_programs.at(m_currentProgramIndex));
    loadModel((getAssetsPath() + "catzinho.obj"),1);
    m_mappingMode = 3;
  }
 // ImGui::NextColumn();
    
  if(ImGui::Button("Coelho", ImVec2(70, 50))){
  ...
  float color = 0.6;
    ...
    loadModel((getAssetsPath() + "bunny.obj"),2);
    m_mappingMode = 0;
    
  m_trackBallModel.setAxis(glm::normalize(glm::vec3(1, 1, 1)));
  }
  //ImGui::NextColumn();
  if(ImGui::Button("Pássaro", ImVec2(70, 50))){
        currentIndex = 0;
        m_currentProgramIndex = currentIndex;
        m_model.setupVAO(m_programs.at(m_currentProgramIndex));
    loadModel((getAssetsPath() + "Bird.obj"),3);
    m_mappingMode = 3;
  }
  ImGui::End();
